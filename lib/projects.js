...
function generateOngoingProjectJQuery(showsData) {
    ...
    const $baseCards = $(`<div class="p-2 bg-white dark:bg-gray-700 rounded shadow-sm" />`);
    const $baseCardsInner = $(`<div class="flex pt-1" />`);
    const $baseContentCards = $(`<div class="flex flex-col pt-1" />`);
    const $baseStatusPills = $(`<div class="flex-row pt-2 text-center flex flex-wrap" />`);
    const $baseImgLoc = $(`<div class="icon w-8/12 p-2" />`);
    const $basePill = $(`<div class="text-sm text-black rounded-full px-3 m-1" />`);
    const $baseAnimeTitle = $(`<div class="text-xl font-bold align-top text-gray-900 dark:text-gray-200" />`);
    ...
    showsData.data.forEach((anime_data) => {
        ...
        let $title = $baseAnimeTitle.clone().text(anime_data.title);
        let $imageCard = $baseImgLoc.clone().append($(`<img />`).attr("src", anime_data.poster));
...
if (!isNone($OverflowRoot)) {
    ...
    overviewStats().then((res) => {
        ...
        if (!isAdmin) {
            // Server type
            collectedCounter.push(new CountUp("ostats-finished", res.data.finished));
            collectedCounter.push(new CountUp("ostats-ongoing", res.data.unfinished));
        } else {
            // Admin type
            collectedCounter.push(new CountUp("ostats-server", res.data.servers));
            collectedCounter.push(new CountUp("ostats-anime", res.data.anime));
            collectedCounter.push(new CountUp("ostats-admin", res.data.admins));
            collectedCounter.push(new CountUp("ostats-project", res.data.project));
        }
        console.info(`[OverviewStats] Collected ${collectedCounter.length} counter`);
        console.info(collectedCounter);
