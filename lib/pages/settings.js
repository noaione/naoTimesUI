...
function resetAdminRemove() {
    $(".show-admin-remove").on("click", function (ev) {
        ev.preventDefault();
        // Destroy self
        $(this).parent().remove();
...
function handleTheOtherAdminButton() {
    ...
    $("#admin-add-btn").on("click", function (ev) {
        ev.preventDefault();
...
function changeToNormalAdminList(adminSets) {
    ...
    adminSets = adminSets.sort();
    for (let i = 0; i < adminSets.length; i++) {
        const $template = $(`<span class="dark:text-white"></span>`);
        $template.text("- " + adminSets[i]);
...
function handleAdminForm() {
    $("#admin-change-btn").on("click", function (e) {
        e.preventDefault();
        const $this = $(this);
...
export function handleSettingsPage() {
    ...
    $("#remove-server-btn").on("click", function (ev) {
        ev.preventDefault();
