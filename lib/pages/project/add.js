...
function injectAfterItemSelect($ProjectAdd, $ProjectEpisode, data) {
    ...
    const posterUrl = coverData.extralarge || coverData.large || coverData.medium;
...
function searchAnimeForProject(selector, $projectSelector, $projectEpisode) {
    const select = new TomSelect(selector, {
        ...
        load(query, callback) {
            const url = `anilist/find?q=${encodeURIComponent(query)}`;
...
export function handleProjectAdd($ProjectAdd) {
    ...
    $("#episode-id").on("keyup", (ev) => {
        ev.preventDefault();
        const $this = $(this);
        const testString = Number.isNaN(parseInt($this.val()));
        if (!testString) {
