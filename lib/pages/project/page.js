...
function createRolePillsJQuery(rolecode, username, userid) {
    ...
    <div class="text-base text-gray-900 items-center flex flex-row" aria-role-show="XX">
        ...
        <span class="px-2 rounded font-semibold">???</span>
    </div>
    `);
    ...
    let uname = username || "Tidak diketahui";
    const splitName = uname.split("#");
    uname = splitName.slice(0, splitName.length === 1 ? 1 : splitName.length - 1);
    $container
        .find("span")
...
function createPartialEpisodeBoxProject(results, episode = "XX") {
    const $container = $(`
    ...
    for (const [role_name, role_stat] of Object.entries(results.progress)) {
        ...
        const $rolePopup = $baseRolePillsPopup.clone().addClass(pillColor).text(titleRole);
        const $textSpan = $("<span />").text(textRole);
        const $rolePill = $baseRolePills
            .clone()
            .addClass(pillColor)
...
function createEpisodeBoxProject(episode_status) {
    ...
    // eslint-disable-next-line no-restricted-syntax
    for (const [role_name, role_stat] of Object.entries(episode_status.progress)) {
        const pillColor = Utils.roleColorPill(role_name);
        const titleRole = Utils.roleToIDUserThingy(role_name);
        const textRole = Utils.roleToPrettyName(role_name);
...
const globalEpisodeCheckHandling = {};
...
function changeIntoCheckboxEpisode($this) {
    ...
    const collectedEpisodeSets = {};
    let $ongoingDiv;
    let $doneDiv;
    const $collectedJQDiv = $(
...
function rehandleEpisodeEditBotan() {
    $(".show-ep-edit").on("click", (e) => {
        ...
        if ($this.attr("data-current-type") === "show") {
            ...
            handleEpisodeEditSubmit(episode, (newEpisodeResult) => {
...
function rehandleStaffEditBotan() {
    ...
    $(".show-edit-role-btn").on("click", (ev) => {
        ev.preventDefault();
        const $btnThis = $(this);
        ...
        if (Number.isNaN(parseInt($inputFlow.val()))) {
            return;
        }
...
function modalErrorPopup(message) {
    $("#modal-error").attr("x-data", `{ showModal: true }`);
    $("p[aria-type=modal-text-error]").text(message);
}

// Botan Alias Click Edit
function rehandleAliasBotanEdit() {
    $(".show-alias-edit").on("click", (ev) => {
        ev.preventDefault();
        const $this = $(this);
        const $AliasParent = $this.parent();
        const aliases = $AliasParent.attr("data-aliases");

        const $CopyOfParent = $AliasParent.clone().addClass("mt-2");
        $CopyOfParent.empty();
        const $btn = $(
            `<button class="show-aliases-btn-submit px-3 py-2 font-semibold bg-green-600 hover:bg-green-700 text-white duration-150 transition-colors rounded mt-2" />`
        ).text("Selesai");
        $CopyOfParent.append(
            $(`<input id="show-aliases-edit-input" autocomplete="off" placeholder="Ketik Alias" />`)
                .val(aliases)
                .attr("data-aliases", aliases)
        );
        $CopyOfParent.append($btn);
        $this.parent().replaceWith($CopyOfParent);
        const $TomSelected = new TomSelect("#show-aliases-edit-input", {
            persist: false,
            createOnBlur: true,
            create: true,
            createFilter(input) {
                const loweredInput = input.toLowerCase();
                return loweredInput.indexOf(" ") <= 1;
            },
        });

        $TomSelected.on("change", (value) => {
            const $aliasesInput = $("#show-aliases-edit-input");
            $aliasesInput.attr("data-aliases", value);
        });
        rehandleAliasBotanEdit();
    });

    $(".show-aliases-btn-submit").on("click", (ev) => {
        ev.preventDefault();
        const $this = $(this);
        const $AliasParent = $this.parent();
        const $aliasesInput = $("#show-aliases-edit-input");
        const newAliases = $aliasesInput.attr("data-aliases").split(",");
        const animeId = $("#root").attr("data-anime-id");
        console.info(`Sending ${animeId} with ${newAliases}`);

        Utils.requestAPISendData("put", "/api/showtimes/alias", {
            animeId,
            aliases: newAliases,
        })
            .then((results) => {
                if (results.code !== 200) {
                    modalErrorPopup(results.message);
                } else {
                    const FixedAliases = results.data;
                    const $CopyOfParent = $AliasParent.clone();
                    $CopyOfParent.empty();
                    if (FixedAliases.length < 1) {
                        $CopyOfParent.append($("<span />").text("Tidak ada alias"));
                        $CopyOfParent.attr("data-aliases", "");
                    } else {
                        $CopyOfParent.attr("data-aliases", FixedAliases.join(","));
                        $CopyOfParent.append($("<span />").text(`Alias: ${FixedAliases.join(", ")}`));
                    }
                    $CopyOfParent.append(
                        $(
                            `<button class="show-alias-edit ml-1 text-red-400 hover:text-red-500 transition-colors duration-150" />`
                        ).text("[Edit]")
                    );
                    $this.parent().replaceWith($CopyOfParent);
                    rehandleAliasBotanEdit();
...
export function handleProjectPage($ProjectPageRoot, PROJECT_DATA) {
    ...
    $projekRoles.empty();
    // eslint-disable-next-line no-restricted-syntax
    for (const [role_name, role_data] of Object.entries(PROJECT_DATA.assignments)) {
    ...
    if (PROJECT_DATA.aliases.length < 1) {
        $projekAlias.append($("<span />").text("Tidak ada alias"));
        $projekAlias.attr("data-aliases", "");
    } else {
        $projekAlias.attr("data-aliases", PROJECT_DATA.aliases.join(","));
        $projekAlias.append($("<span />").text(`Alias: ${PROJECT_DATA.aliases.join(", ")}`));
    }
    $projekAlias.append(
        $(
            `<button class="show-alias-edit ml-1 text-red-400 hover:text-red-500 transition-colors duration-150" />`
