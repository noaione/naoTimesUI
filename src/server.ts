...
if (process.env.NODE_ENV === "development") {
    ...
    // app.use(connect_livereload({ port: 3529 }));
}
...
const sessionMiddleware = express_session({
    ...
    store: new RedisStore({ client: RedisClient as IORedis.Redis }),
});
...
const SOCKET_PASSWD = process.env.BOT_SOCKET_PASSWORD;

export { app, sessionMiddleware, SOCKET_PASSWD, logger };
